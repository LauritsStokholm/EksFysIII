...
accelerator \cref{fig_setup3} was used. The variety of incomming beam particles
...
$\mathrm{{H_2}^{+}}$, as the source was stationary, and not changed.
...
\end{figure}
%
Acceleration of the ions was controllable on the dashboard 
\cref{fig_setup2} by changing the voltage drop across the accelerating tube ,
and thus also the kinetic energy of the incomming beam. This will be described
furhter in the section Procedure.
...
possible incomming ions was deflected to enter the beamline, thus interact with the target.
...
\end{align}
%
By assuming that the mass of the two ions are related by $m(\mathrm{H^+}) =
2m({\mathrm{H_2}^+})$, and using $v(X) = \sqrt{\frac{2E}{m(X)}}$ for each ion.
%
